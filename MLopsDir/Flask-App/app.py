...
<body>
    <h1>CPU & Memory Usage (Latest 5)</h1>
    <table>
...
def data():
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT time, cpu, memory FROM metrics ORDER BY time DESC;")
            rows = cur.fetchall()
            result = [{"time": row[0].isoformat(), "cpu": row[1], "memory": row[2]} for row in rows]
